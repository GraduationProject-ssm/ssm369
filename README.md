...
# 绪论
## 1.1 研究背景
当前社会各行业领域竞争压力非常大，随着当前时代的信息化，科学化发展，让社会各行业领域都争相使用新的信息技术，对行业内的各种相关数据进行科学化，规范化管理。这样的大环境让那些止步不前，不接受信息改革带来的信息技术的企业随时面临被淘汰，被取代的风险。所以当今，各个行业领域，不管是传统的教育行业，餐饮行业，还是旅游行业，医疗行业等领域都将使用新的信息技术进行信息革命，改变传统的纸质化，需要人手工处理工作事务的办公环境。软件信息技术能够覆盖社会各行业领域是时代的发展要求，各种数据以及文件真正实现电子化是信息社会发展的不可逆转的必然趋势。本星火许电影小说网站也是紧跟科学技术的发展，运用当今一流的软件技术实现软件系统的开发，让小说和电影信息完全通过管理系统实现科学化，规范化，程序化管理。从而帮助信息管理者节省事务处理的时间，降低数据处理的错误率，对于基础数据的管理水平可以起到促进作用，也从一定程度上对随意的业务管理工作进行了避免，同时，星火许电影小说网站的数据库里面存储的各种动态信息，也为上层管理人员作出重大决策提供了大量的事实依据。总之，星火许电影小说网站是一款可以真正提升管理者的办公效率的软件系统。
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此星火许电影小说网站为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
# 4 系统设计
星火许电影小说网站的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
图4.1 程序工作的原理图
## 4.2 系统功能结构设计
在分析并得出使用者对程序的功能要求时，就可以进行程序设计了。如图4.2展示的就是管理员功能结构图，管理员主要负责填充小说和其类别信息，并对已填充的数据进行维护，包括修改与删除，管理员也需要对电影,对评论,对收藏,对新闻等进行管理。
...
|3|dianying\_types|Integer|电影类型|是|
|4|dianying\_photo|String|电影图片|是|
|5|dianying\_video|String|电影视频|是|
|6|dianying\_content|String|电影详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|12|dianying\_id|Integer|电影|是|
|13|yonghu\_id|Integer|用户|是|
|14|dianying\_liuyan\_content|String|评论内容|是|
|15|reply\_content|String|回复内容|是|
|16|insert\_time|Date|评论时间|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|18|dic\_code|String|字段|是|
|19|dic\_name|String|字段名|是|
|20|code\_index|Integer|编码|是|
|21|index\_name|String|编码名字|是|
|22|super\_id|Integer|父字段id|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|24|addtime|Date|创建时间|是|
|25|title|String|帖子标题|是|
|26|content|String|帖子内容|是|
|27|parentid|Integer|父节点id|是|
|28|userid|Integer|用户id|是|
|29|isdone|String|状态|是|
表4.6新闻表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|30|news\_name|String|新闻名称|是|
|31|news\_types|Integer|新闻类型|是|
|32|news\_photo|String|新闻图片|是|
|33|insert\_time|Date|新闻时间|是|
|34|news\_content|String|新闻详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|36|single\_seach\_name|String|名字|是|
|37|single\_seach\_types|Integer|数据类型|是|
|38|single\_seach\_content|String|内容|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|40|xiaoshuo\_name|String|小说名称|是|
|41|xiaoshuo\_types|Integer|小说类型|是|
|42|xiaoshuo\_photo|String|小说封面|是|
|43|xiaoshuo\_file|String|小说文件|是|
|44|xiaoshuo\_content|String|小说介绍|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|50|xiaoshuo\_id|Integer|小说|是|
|51|yonghu\_id|Integer|用户|是|
|52|xiaoshuo\_liuyan\_content|String|评论内容|是|
|53|reply\_content|String|回复内容|是|
|54|insert\_time|Date|评论时间|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|56|yonghu\_name|String|姓名|是|
|57|yonghu\_phone|String|手机号|是|
|58|yonghu\_id\_number|String|身份证号|是|
|59|yuangong\_photo|String|照片|是|
|60|new\_money|BigDecimal|余额|是|
|61|huiyuan\_types|Integer|是否会员|是|
|62|create\_time|Date|创建时间|是|
表4.12管理员表表
...
图5.2 新闻管理页面
### 5.1.3 电影管理
如图5.3显示的就是电影管理页面，此页面提供给管理员的功能有：新增电影,修改电影,删除电影。

![](/md/blog.012.png)

图5.3 电影管理页面
### 5.1.4 电影类型管理
如图5.4显示的就是电影类型管理页面，此页面提供给管理员的功能有：新增类型,修改类型,删除类型。
...
## 5.2前台首页功能模块
` `星火许电影小说网站，在星火许电影小说网站可以查看电影信息,小说信息、我的、跳转到后台、公司介绍等内容，如图5-17所示。
...
电影信息，在电影信息页面可以查看电影的名字,电影的类型,电影的视频，如图5-19所示。 
